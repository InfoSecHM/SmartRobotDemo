...
import filters.core.*;
import filters.core.actions.Action;
import filters.core.objects.GridRobotBelief;

import java.util.ArrayList;
import java.util.List;

import static filters.core.GridWorldDomain.ATTX;
import static filters.core.GridWorldDomain.ATTY;
import static filters.core.GridWorldDomain.ATT_THETA;
...
public class MotionControllerModule implements ObjectInstance {
	...
	protected State move(State s, RobotControl action) {
		...
		int ad = agent.getIntValForAttribute(ATT_THETA);
		// TODO theta

		// Trim values if wold is cyclic
		int nd = GridWorldDomain.INSTANCE.trimValue(ATT_THETA, ad + theta, true);
		int[] dcomp = GridLocation.getGridLocation(dist, nd);
		int nx = GridWorldDomain.INSTANCE.trimValue(ATTX, ax + dcomp[0], false);
		int ny = GridWorldDomain.INSTANCE.trimValue(ATTY, ay + dcomp[1], false);
		...
		s = s.setObjectsValue(agent.getName(), ATT_THETA, nd);
