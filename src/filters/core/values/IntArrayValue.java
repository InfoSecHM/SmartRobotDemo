 ...
 * In general, it is reccomended that a series of {@link IntValue} attributes is
 * defined instead of using this class, because a series of {@link IntValue}s
 * will have better compatibility with existing BURLAP tools and algorithms, but
 * this class can be used in cases where there is a very large number of int
 * values that have to be stored in each state to cut down on memory overhead.
 * 
