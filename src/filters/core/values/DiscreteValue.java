 ...
 * The int values correspond to the attributes categorical list of discrete
 * values, so the int values should always be &gt;= 0 unless it is unset, which
 * is specified by a value of -1.
 *
...
public class DiscreteValue extends OOMDPValue implements Value {
    ...
    /**
     * The discrete value stored as an integer. If the attribute defines
     * categorical values with strings, this int value represents the index in
     * the list of categorical values. The default value of -1 indicates an
