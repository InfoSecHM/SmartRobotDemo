 ...
 * There are different types of attributes to support different types of value.
 * Currently, there are attributes to support discrete values, real values,
 * string values, and relational values. There are two different kinds of
 * discrete values, int and disc. While both are internally stored as integers,
 * the int value may be unbounded in range, whereas disc values have a finite
 * domain and also refer to specific categorical values represented as string.
 * Discrete attributes require a finite domain (that is, it must be specified
 * how many different discrete values there are). There are two different kinds
 * of real attributes; bounded and unbounded. A a bounded real attribute should
 * have a lower limit value and an upper limit value. Unbounded real attributes
 * may span any range. Both attributes' values are represented by double data
 * types. There are also two kinds of relational attributes: single target and
 * multi-target. A single target relational attribute can only be linked to one
 * other object, whereas a multi-target relational attribute can be linked to a
 * set of other objects. In both cases, the relational attribute can be unset to
 * any target. The single-target relational attribute values are represented by
 * strings that specify the name reference of the object to which they are
 * connected. Multi-target relational attribute values are represented by an
 * ordered set of string values indicating the targets to which the attribute is
 * connected.
 *
 * 
...
public class Attribute {
    ...
    /**
     * And enumeration type to indicate the various types of attributes
     * supported. There is an special value for an unspecified TYPE, but in
     * general this should not be used unless creation of values for it is
     * handled with care. The integer correspondence of attributes types are:
     * -1: NOTYPE (no type) 0: DISC (discrete) 1: REAL (bounded real) 2:
     * REALUNBOUND (unbounded real) 3: RELATIONAL (single-target relational) 4:
     * MULTITARGETRELATIONAL (multi-target relation) 5: INT (discrete int, but
     * not categorical) 6: BOOLEAN (0, or 1, discrete value for false, true) 7:
     * STRING (a string) 8: INTARRAY (an attribute whose value is an int[] type)
    ...
    public enum AttributeType {
        NOTYPE(-1), DISC(0), REAL(1), REALUNBOUND(2), RELATIONAL(3), MULTITARGETRELATIONAL(4), INT(5), BOOLEAN(6), STRING(
                7), INTARRAY(8), DOUBLEARRAY(9);
    ...
    /**
     * Constructs an attribute with an unspecified type that will need to be
     * specified later.
     * 
     * @param domain
     *            the domain that the attribute exists in
     * @param name
    ...
    /**
     * Constructs with a given attribute type. If the attribute type is set to
     ...
     *            the name of the attribute
     * @param type
     *            the type of the attribute (discrete or real)
     */
    ...
    /**
     * Will create a new Attribute object with the same configuration and name
     * as this one.
     * 
     * @param newDomain
     *            the domain to which the new attribute should be attached
     ...
     * 
     * @param vals
     *            the list of categorical values for this discrete attribute
     */
     ...
     * 
     * @param vals
     *            an array of categorical values for this discerte attribute
     */
    ...
    /**
     * Sets the possible range of discrete (@link {@link AttributeType#DISC})
     * values for the attribute. The categorical values will be set to the the
     * string representation of each integer number.
     * 
     * @param low
     *            the minimum int value for the attribute
     * @param high
     *            the maximum int value for the attribute
     * @param step
     *            the amount by which the int value will increase
     */
     ...
     * 
     * @return a Value object compatible with this Attributes type (i.e.,
     *         discrete or real)
     */
